...
sub check_pass {
  ...
  if ($formdata{new_password} ne $formdata{repeat_password}) {
  ...
  } elsif ($formdata{new_password} !~ /^[\w]{4,8}$/) {
    ...
    ## inquiry to PAM
    ref(my $pamh = new Authen::PAM($service, $ENV{REMOTE_USER}, \&my_conv_func)) or
      die "Error code during PAM init!\n";
...
sub modify_pass {
  ...
  my @info = mgetpwnam($ENV{REMOTE_USER});

  ## get the salt 
  my $salt = substr($info[1], 0, 1);

  ## encrypt the new password with md5
  my $newpass = unix_md5_crypt($formdata{new_password}, $salt);

  ## change the array of user info to include new password
  $info[1] = $newpass;

  ## change the /etc/shadow info and return error if fails
  my $err = modpwinfo(@info);
